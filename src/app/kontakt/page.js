          ...
          <h2 className="contact-form-label p-1 mb-1 uppercase tracking-wide text-white font-semibold text-xl">{"formularz kontaktowy"}</h2>
            ...
            <div className="contact-form-user-data grid grid-cols-1 md:grid-cols-2 gap-3 mb-3">
              <div className='contact-fullName-container flex flex-col'>
                <input className={`contact-input-fullName p-[6px] border-2 border-primary-800 focus:border-secondary-200 focus:outline-none rounded-md ${printFullNameError && 'invalid-field'}`} type="text" name="fullName" placeholder="Imię i nazwisko *" onChange={onInputChange} />
                <label className='contact-label-fullName text-xs text-red-600' htmlFor="fullName" >{printFullNameError && fieldValidity.fullName.message.join(' ')}</label>
              </div>

              <div className='contact-email-container flex flex-col'>
                <input className={`contact-input-email p-[6px] border-2 border-primary-800 focus:border-secondary-200 focus:outline-none rounded-md ${printEmailError && 'invalid-field'}`} type="text" name="email" placeholder="Email *" onChange={onInputChange} />
                <label className='contact-label-email text-xs text-red-600' htmlFor="email" >{printEmailError && fieldValidity.email.message.join(' ')}</label>
              </div>

              <div className='contact-phone-container flex flex-col'>
                <input className={`contact-input-phone p-[6px] border-2 border-primary-800 focus:border-secondary-200 focus:outline-none rounded-md ${printPhoneError && 'invalid-field'}`} type="text" name="phone" placeholder="Telefon *" onChange={onInputChange} />
                <label className='contact-label-phone text-xs text-red-600' htmlFor="phone" >{printPhoneError && fieldValidity.phone.message.join(' ')}</label>
              </div>

              <div className='contact-topic-container flex flex-col'>
                <input className={`contact-input-topic p-[6px] border-2 border-primary-800 focus:border-secondary-200 focus:outline-none rounded-md ${printTopicError && 'invalid-field'}`} type="text" name="topic" placeholder="Temat *" onChange={onInputChange} />
                <label className='contact-label-topic text-xs text-red-600' htmlFor="topic" >{printTopicError && fieldValidity.topic.message.join(' ')}</label>
              </div>
            ...
            <div className='contact-content-container flex flex-col'>
              <textarea className={`contact-input-content w-full h-48 p-[6px] border-2 border-primary-800 focus:border-secondary-200 focus:outline-none rounded-md ${printContentError && 'invalid-field'}`} type="text" name="content" placeholder="Wiadomość *" onChange={onInputChange} />
              <label className='contact-label-content text-xs text-red-600' htmlFor="content" >{printContentError && fieldValidity.content.message.join(' ')}</label>
            </div>
            ...
            <div className='rodo-checkbox-container flex flex-col mr-4'>
              <div className={`contact-accept-rodo flex flex-row items-center h-[76px] p-2  bg-white border-2 rounded-[4px] ${printAcceptRodoError && 'invalid-field'}`}
